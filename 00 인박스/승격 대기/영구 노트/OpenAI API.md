...
- [[SKN26 3차 PICKLE 맛집 추천 챗봇]]은 `ChatOpenAI`로 router와 generator를 호출하고, `text-embedding-3-small`로 식당 관련 텍스트를 임베딩했다.
- [[SKN26 4차 LG Home AI 가전 상담]]은 `gpt-4o-mini`로 제품군 분류, 후속 질문 판별, 슬롯 추출, 최종 답변을 만들고, OpenAI embedding으로 Pinecone 매뉴얼 검색 query vector를 만든다.
