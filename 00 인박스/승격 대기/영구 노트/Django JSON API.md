...
[[SKN26 4차 LG Home AI 가전 상담]]의 공식 프로젝트에서는 `api\views.py`의 `send_chat`, `favorite`, `check_favorite`가 JSON 응답을 반환한다. 개인 확장 `4th_project_change_react`에서는 검색, 상품 상세, 채팅, 계정 기능을 SPA용 JSON API로 더 넓게 분리했다.
