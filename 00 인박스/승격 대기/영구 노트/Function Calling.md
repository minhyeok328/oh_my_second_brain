...
[[SKN26 3차 PICKLE 맛집 추천 챗봇]]의 `slot_extractor.py`는 embedding 경로와 fixed 경로에 다른 schema를 두고, LangGraph node에서 `.invoke()`로 슬롯을 추출한다. [[SKN26 4차 LG Home AI 가전 상담]]은 Pydantic structured output으로 제품군, 후속 질문, 조건 슬롯을 추출한다.
