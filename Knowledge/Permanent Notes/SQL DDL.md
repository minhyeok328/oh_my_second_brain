...
테이블, 데이터베이스, 인덱스 같은 구조를 정의하는 SQL 명령 집합이다.

## 왜 중요한가

스키마는 데이터 품질의 첫 방어선이다. 컬럼 타입과 제약조건을 잘못 잡으면 애플리케이션 코드가 복잡해진다.

## 핵심 개념

- `CREATE`, `ALTER`, `DROP`이 핵심이다.
- 컬럼 타입과 제약조건을 정의한다.
- 스키마 변경은 데이터 마이그레이션과 함께 고려한다.
...
서비스 초기 설계, 분석용 임시 테이블, Django migration 결과 이해에 필요하다.
...
SQL DDL은/는 강의에서 나온 개념을 정의, 사용 조건, 주의점, 연결 개념으로 다시 설명하기 위한 LLM wiki 원자 노트다. 단순 암기보다 실제 문제에서 언제 꺼내 쓸지 판단하는 데 초점을 둔다.

## 언제 쓰는가

- 강의 실습 코드에서 같은 개념을 다시 만날 때
- 프로젝트에서 관련 오류나 설계 결정을 설명해야 할 때
- [[관계형 데이터베이스]], [[SQL 제약조건]], [[Django Migration]]와 함께 문제 원인을 좁힐 때
...
- SQL DDL을/를 쓰면 어떤 문제를 더 단순하게 설명할 수 있는가?
