...
태그: #git #devops #llm_wiki

## 한 줄 정의

파일 변경 이력을 스냅샷으로 기록하고 브랜치로 작업을 분리하는 버전 관리 시스템이다.

## 왜 중요한가

코드와 지식 산출물의 변경 이유를 추적할 수 있어 협업과 복구의 기반이 된다.

## 핵심 개념

- commit은 변경 이력의 단위다.
- branch는 독립 작업 흐름이다.
- merge와 rebase로 변경을 통합한다.
...
프로젝트 관리, 강의 코드 실습, 지식 vault 버전 관리에 사용한다.
...
Git은/는 강의에서 나온 개념을 정의, 사용 조건, 주의점, 연결 개념으로 다시 설명하기 위한 LLM wiki 원자 노트다. 단순 암기보다 실제 문제에서 언제 꺼내 쓸지 판단하는 데 초점을 둔다.

## 언제 쓰는가

- 강의 실습 코드에서 같은 개념을 다시 만날 때
- 프로젝트에서 관련 오류나 설계 결정을 설명해야 할 때
- [[GitHub]], [[Dockerfile]], [[CI CD]]와 함께 문제 원인을 좁힐 때
...
- Git을/를 쓰면 어떤 문제를 더 단순하게 설명할 수 있는가?
