...
HTML에 변수, 조건문, 반복문을 넣어 서버에서 동적으로 렌더링하는 템플릿 시스템이다.

## 왜 중요한가

서버가 만든 데이터를 사용자에게 보이는 HTML로 변환한다.

## 핵심 개념

- `{{ variable }}`로 값을 출력한다.
- `{% for %}`, `{% if %}`로 흐름을 제어한다.
- base template과 block으로 레이아웃을 재사용한다.
...
게시판 목록, 로그인 화면, QnA 상세 페이지, 챗봇 UI에 사용한다.
...
자료 힌트: 11_web_server_workspace/_02_django_template

## 내 말로 다시 설명

Django Template은/는 강의에서 나온 개념을 정의, 사용 조건, 주의점, 연결 개념으로 다시 설명하기 위한 LLM wiki 원자 노트다. 단순 암기보다 실제 문제에서 언제 꺼내 쓸지 판단하는 데 초점을 둔다.

## 언제 쓰는가

- 강의 실습 코드에서 같은 개념을 다시 만날 때
- 프로젝트에서 관련 오류나 설계 결정을 설명해야 할 때
- [[HTML]], [[Django View]], [[Django Static과 Media]]와 함께 문제 원인을 좁힐 때
...
- Django Template을/를 쓰면 어떤 문제를 더 단순하게 설명할 수 있는가?
