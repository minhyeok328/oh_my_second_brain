...
LangChain 컴포넌트를 공통 실행 인터페이스로 연결하는 추상화다.

## 왜 중요한가

프롬프트, 모델, 파서, 검색기를 파이프처럼 조합해 실험 단계를 명확하게 만든다.
...
RAG 체인, 요약 체인, 분류 파이프라인 구성에 사용한다.
...
Runnable은/는 강의에서 나온 개념을 정의, 사용 조건, 주의점, 연결 개념으로 다시 설명하기 위한 LLM wiki 원자 노트다. 단순 암기보다 실제 문제에서 언제 꺼내 쓸지 판단하는 데 초점을 둔다.

## 언제 쓰는가

- 강의 실습 코드에서 같은 개념을 다시 만날 때
- 프로젝트에서 관련 오류나 설계 결정을 설명해야 할 때
- [[LangChain]], [[Retriever]], [[Output Parser]]와 함께 문제 원인을 좁힐 때
...
- Runnable을/를 쓰면 어떤 문제를 더 단순하게 설명할 수 있는가?
