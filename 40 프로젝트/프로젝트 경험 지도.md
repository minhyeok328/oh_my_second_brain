...
- 승격된 지식: [[Streamlit Session State]], [[MySQL Connector Python]], [[SQL SELECT와 WHERE]], [[HTTP 요청은 타임아웃과 실패 처리를 기본값으로 가져야 한다]]
...
- 승격된 지식: [[분류 평가 지표]], [[Train Test Split]], [[Feature Engineering]], [[MLflow]], [[FastAPI는 타입 선언을 요청 검증과 문서화에 재사용한다]], [[XGBoost]], [[Docker Compose]]
